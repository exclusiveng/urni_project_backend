...
export const createBranch = async (req: AuthRequest, res: Response): Promise<Response | void> => {
  try {
    ...
    await NotificationService.notifyAdmins(
      req,
      "New Branch Created",
      `${(req as any).user?.name || 'An admin'} created branch ${savedBranch.name}`,
...
export const updateBranch = async (req: Request, res: Response): Promise<Response | void> => {
  try {
    ...
    await NotificationService.notifyAdmins(
      req,
      "Branch Updated",
      `Branch ${branch.name} was updated by ${(req as any).user?.name || 'an admin'}.`,
      { branchId: branch.id }
    );
...
export const deleteBranch = async (req: AuthRequest, res: Response): Promise<Response | void> => {
  try {
    ...
    await NotificationService.notifyAdmins(
      req,
      "Branch Deleted",
      `Branch ${branchToDelete.name} was deleted by ${(req as any).user?.name || 'an admin'}.`,
